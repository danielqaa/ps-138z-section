...
We can also use `ggplot2`, a very popular and very powerful data visualization package. First, we have to load the package (we installed by default it when we installed the `tidyverse` before): 
...
Note: the arguments within `geom_point()` are optional (i.e., not strictly required for `ggplot` to produde the scatterplot)
...
This was a short introduction to `ggplot2`. We will stop here. In two weeks, we will continue learning how to use this package.  
